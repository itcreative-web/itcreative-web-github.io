...
window.onload = function () {
  ...
  for (var j = 0; j < period.length; j++) {
    period[j].innerHTML = "\u0111\u1EBFn ng\xE0y ".concat(day, ".").concat(month, ".").concat(year);
  }
  ...
  var bgColor = $('.catalog__content-img'),
      catalogTitle = document.querySelector('p.color output'),
  ...
  $('.catalog__content .color figure').on('click', function () {
    ...
    if (activeColor === 'blue') {
      ...
      catalogTitle.innerText = 'Xanh da trời xám';
    } else if (activeColor === 'gray') {
      bgColor.removeClass('blue red').addClass('gray');
      catalogTitle.innerText = 'xám';
    } else if (activeColor === 'red') {
      bgColor.removeClass('blue gray').addClass('red');
      catalogTitle.innerText = 'đỏ';
    }
  });
  /*
      the same height
