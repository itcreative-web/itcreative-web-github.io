$(function () {
	...
	$(window).scroll(function(){
		var offset = $('.slider__section').offset().top - 700
		if ($(this).scrollTop() > offset) {
			$('.download__line').css('width','98%');
		}
	});

	function modal(selector, openBtn, close) {
		...
		$(selector).click(function(e) {
			if(e.target.classList.contains("modal") || e.target.classList.contains("modal__confidantion-wrapper")) {
				$(this).removeClass("active")
			}
		})
	}

	modal(".modal", ".header__order-btn", ".modal__close")
	modal(".modal__confidantion", ".confidential", ".modal__close")
	...
	function quantity() {
		...
		if(localStorage.getItem("quantity")) {
			$(".header__remaining-goods").text(localStorage.getItem("quantity"));
		} else {
			currentNumber = 25
			localStorage.setItem("quantity", currentNumber)
			$(".header__remaining-goods").text(currentNumber);
		}
